function Set-IBTrustAllCertsPolicy { 
    <#
    .SYNOPSIS
    ...
    $IDS_SCRIPTNAME = 'Set-IBTrustAllCertsPolicy'


    if([System.Net.ServicePointManager]::CertificatePolicy.ToString() -eq "TrustAllCertsPolicy")
    {
        Write-Verbose "$IDS_SCRIPTNAME : Current policy is already set to TrustAllCertsPolicy"
    }
    else
    {
        add-type @"

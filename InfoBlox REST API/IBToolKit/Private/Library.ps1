...
function ProcessError() 
{
    ...
    switch($ErrorNum) {
        ...
        '002' { Write-Output "$IDS_ORIGINATING_SCRIPTNAME : $ErrorMessage, Use Import-IBSServerCertificate for importing the Server Certificate `nor use Set-IBTrustAllCertsPolicy"}
        '003' { Write-Output "$IDS_ORIGINATING_SCRIPTNAME : Encrypted string not valid anymore, maybe it was created in another session, use Initialize-IBRestAPI"}
